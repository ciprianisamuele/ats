...
Three important structures are trend, seasonality and stochastic serial dependence.
...
\]
Hence all finite-dimensional distributions are invariant under time shifts. Whenever the corresponding moments exist, in particular \(E[X_t]=\mu\), \(\operatorname{Var}(X_t)=\sigma^2\), and \(\operatorname{Cov}(X_t,X_{t+h})=\gamma(h)\), so covariance depends only on the \key{lag} \(h\).
...
\key{Assessing stationarity.} Stationarity cannot be proven from a single realization; it is treated as a working hypothesis. Typical violations: trend, changing variance, deterministic seasonality, structural breaks or a dependence structure that changes over time. A useful diagnostic is to split a long series into consecutive segments and compare their plots and correlograms.
...
A visible stochastic cycle does \key{not} imply that its period should be used as frequency. Frequency must be chosen using domain knowledge. If the \texttt{ts} class can store only one of several plausible cycles, use the strongest or most relevant one; irregular annual counts may be represented by a sensible integer (e.g. 252 trading days) or an average, possibly non-integer, frequency. For high-frequency data, multiple seasonalities may coexist, e.g. daily + weekly + yearly.
...
\key{Seasonal displays.} A season plot overlays successive cycles against the within-cycle position, making the seasonal shape and its evolution visible. A subseries/month plot instead groups all observations from the same season across years and usually marks their season-specific mean; it is especially useful for comparing the strength and stability of different seasonal effects.
...
The estimated seasonal effects are conventionally centered to have mean (and hence sum over one cycle) zero, so the overall level remains in the trend. Classical \texttt{decompose()} also requires at least two complete seasonal cycles.
...
STL = \key{Seasonal-Trend decomposition using LOESS}; it decomposes \(X_t=\text{trend}+\text{seasonal}+\text{remainder}\). Both trend and seasonality are obtained by smoothing. Unlike centered moving-average decomposition, STL returns components with the same length as the original series; however, because it is model-free, extrapolating its smooth trend and seasonal components is not automatic.
...
Robust regression can reduce the influence of outliers in a parametric decomposition. More flexible model:
\[
X_t=f(t)+\alpha_{\text{season}(t)}+R_t,
\]
with smooth trend \(f(t)\) and seasonal dummy effects. For GAM-type trends, smoothness/effective degrees of freedom should be selected from the data, commonly by cross-validation. Very high-order polynomial trends should be avoided because of poor boundary behavior.
...
In the simple linear regression of \(X_t\) on \(X_{t-k}\) alone, \(\rho(k)^2\) is the fraction of variance explained by that linear association. This interpretation is conditional on the simple linear setting: it does not measure nonlinear dependence or the additional predictive contribution after other lags are included.
...
A \key{random walk} is obtained when the first difference is White Noise:
\[
\boxed{(1-B)X_t=E_t\quad\Longleftrightarrow\quad X_t=X_{t-1}+E_t.}
\]
Iterating gives \(X_t=X_0+\sum_{j=1}^tE_j\). Hence, for independent innovations with variance \(\sigma_E^2\),
\[
E[X_t]=E[X_0],\qquad
\operatorname{Var}(X_t)=\operatorname{Var}(X_0)+t\sigma_E^2,
\]
so the process is non-stationary even though its differences are stationary White Noise. It is the boundary AR(1) case \(\alpha=1\), with a unit root.

With a constant in the differenced equation,
\[
\boxed{X_t=c+X_{t-1}+E_t,}
\]
the process is a random walk with \key{drift}: \(E[X_t]=E[X_0]+ct\). Thus a non-zero mean of the first-differenced process becomes a linear drift in levels; it is not automatically present and must be included/estimated explicitly.
...
SARIMA extends ARIMA to seasonal non-stationarity. For seasonal period \(s\), seasonal differencing is
\[
...
\item Residuals must resemble approximately Gaussian White Noise; if multiple adequate candidates remain, use AIC/parsimony.
\end{enumerate}
...
Invertible AR, MA and ARMA processes are linear because they can be represented as linear combinations of innovations. Models containing nonlinear functions such as squared past observations are nonlinear. Such models are useful, for example, when rises and falls are asymmetric or when dependence appears in volatility rather than in the conditional mean.
...
Typical empirical signs are volatility clustering, heavy tails in a Normal Q--Q plot, negligible autocorrelation in returns and persistent autocorrelation in squared returns.
...
After fitting, form the standardized residuals
\[
\widehat W_t=\frac{\widehat E_t}{\widehat\sigma_t}.
\]
Then inspect both:
\[
\boxed{
ACF/PACF(\widehat W_t)
\quad\text{and}\quad
ACF/PACF(\widehat W_t^2).
}
\]
...
\]
The error process should be stationary, but may be serially correlated and therefore need not be White Noise. These conditions describe an input/output system: the predictors may affect the response, but the response must not feed back into the predictors.
...
\]
Its uncertainty is the conditional forecast variance
\[
v_{n+k|n}=\operatorname{Var}(X_{n+k}\mid X_{1:n}),
\]
which, under a Gaussian or approximate-Gaussian forecast distribution, gives the pointwise 95\% interval \(\widehat X_{n+k|n}\pm1.96\sqrt{v_{n+k|n}}\).
...
Pure time-series forecasts use the past of the modeled series. External predictors can be incorporated only if their future values are known or forecast too; otherwise the result is better treated as a baseline for scenarios or transparent expert adjustment.
...
\[
\boxed{\begin{array}{c}
\text{forecast intervals may omit uncertainty}\\
\text{from estimated drift/trend}\\
\Rightarrow\text{optimistic extrapolation intervals}
\end{array}}
\]
...
\[
a_t=\sum_{i=0}^{t-1}\alpha(1-\alpha)^i x_{t-i}+(1-\alpha)^t a_0.
\]
Thus the initial level receives a geometrically vanishing weight; for a long series this yields the usual approximate infinite weighted average.
...
\boxed{
\begin{array}{c|c}
\text{Model} & \text{long-horizon behavior}\\ \hline
ARMA & \widehat X\to m,\quad \text{PI width}\to\text{constant}\\
ARIMA(d=1) & \widehat X\to\text{constant without drift},\quad\text{PI}\uparrow\infty\\
ARIMA+\text{drift} & \widehat X\text{ linear};\ \text{drift uncertainty may be omitted}\\
SARIMA & \text{trend/season extrapolated},\quad\text{PI}\uparrow\\
SES & \widehat X\to\text{smoothed level}\\
\text{Holt--Winters} & \widehat X=\text{level+trend+season}
\end{array}}
\]
...
X_{1,t},\qquad X_{2,t}.
\]
Multivariate analysis studies their within-series and cross-series dependence with the series initially on \key{equal footing}. This differs from time-series regression, which designates response and explanatory variables and posits a directional relation. All theory here assumes stationary series; if necessary, transform/difference first.

The scientific question is often whether changes in one series are associated with earlier/later changes in another, and with what delay. Cross-correlation alone establishes temporal linear association, not causality.
...
\textbf{Case 1: no correlation between the two series for large lags.} If
\(\rho_{12}(j)=0\) for \(|j|\ge m\), then for \(|k|\ge m\),
\[
\boxed{
\operatorname{Var}(\widehat\rho_{12}(k))
\approx\frac1n\sum_{j=-\infty}^{\infty}
\left\{\rho_{11}(j)\rho_{22}(j)+
\rho_{12}(j+k)\rho_{12}(j-k)\right\}.
}
\]
The variance still depends on all auto- and cross-correlations of both processes.
...
Under the null of no cross-dependence, prewhitened series have negligible autocorrelation, so the ordinary \(\pm2/\sqrt n\) cross-correlation bounds become approximately valid. Whitening removes within-series dependence; it does \emph{not} remove genuine cross-dependence.

The filtered and original cross-covariances satisfy
\[
\gamma_{UV}(k)=\sum_{i=0}^{\infty}\sum_{j=0}^{\infty}a_i b_j\,
\gamma_{12}(k+i-j).
\]
For invertible filters this implies
\[
\boxed{\rho_{UV}(k)=0\ \forall k\quad\Longleftrightarrow\quad
\rho_{12}(k)=0\ \forall k.}
\]
...
\[
\boxed{\begin{array}{c}
\text{never interpret a raw cross-correlogram}\\
\text{when the series are autocorrelated}
\end{array}}
\]
...
\text{linear association} & \text{directional model}\\
\text{positive/negative lags} & \text{causal lags }j\ge0\\
\text{prewhiten both series} & \text{input filter also on output}\\
\text{raw magnitude is mixed} & \nu_j=\text{lag effect}
\end{array}}
\]
...
% ============================================================
\section{Vector Autoregressions (VAR)}

Use a VAR when several series influence one another and no one-way input--output structure is defensible. For \(K\) series collected in \(Y_t\in\mathbb R^K\), a VAR(\(p\)) is
\[
\boxed{Y_t=c+\Phi_1Y_{t-1}+\cdots+\Phi_pY_{t-p}+E_t.}
\]
It contains one regression equation per component. The vector innovations \(E_t\) are serially uncorrelated with mean zero, but their contemporaneous covariance matrix \(\Sigma_E\) need not be diagonal: shocks to different series may occur together.

A VAR must be fitted to stationary series: use a \key{VAR in levels} when levels are stationary and a \key{VAR in differences} after suitable differencing otherwise. With an intercept, the system contains \(K+pK^2\) coefficients, so order growth is costly. Select \(p\) using AIC, HQ, BIC/SC or FPE; BIC is often preferred for a parsimonious model.

Model adequacy requires the residual vector to be multivariate White Noise. Inspect residual auto- and cross-correlations and use a multivariate Portmanteau test; a large p-value means failure to reject residual serial independence, not proof that the model is correct.

Forecast recursively:
\[
\boxed{\widehat Y_{n+h|n}=c+\sum_{j=1}^{p}\Phi_j
\widetilde Y_{n+h-j},\qquad
\widetilde Y_t=Y_t\ (t\le n),\quad
\widetilde Y_t=\widehat Y_{t|n}\ (t>n).}
\]
Thus forecasts of all components are produced jointly and each may use lagged values of every series.
...
\item Forecast uncertainty: \(v_{n+k|n}=\operatorname{Var}(X_{n+k}\mid X_{1:n})\); Gaussian 95\% PI is forecast \(\pm1.96\sqrt v\).
...
\item VAR models reciprocal multivariate dynamics: \(Y_t=c+\sum_{j=1}^p\Phi_jY_{t-j}+E_t\).
\item VAR requires stationary inputs; select lag order parsimoniously and check residual auto- and cross-correlation.
\end{enumerate}
...
Here \(\nu\) is measured in cycles per observation. With sampling interval \(\Delta t\), physical frequency and Nyquist frequency are
\[
\boxed{f=\nu/\Delta t,\qquad f_N=1/(2\Delta t)},
\]
and the physical period is \(T=\Delta t/\nu=1/f\). Frequencies differing by integer multiples of \(1/\Delta t\) are aliases.
...
More precisely, spectrum and autocovariance form a Fourier pair:
\[
\boxed{f(\nu)=\sum_{h=-\infty}^{\infty}\gamma(h)e^{-2\pi i\nu h},\qquad
\gamma(h)=\int_{-1/2}^{1/2}f(\nu)e^{2\pi i\nu h}\,d\nu.}
\]
For a real-valued process, \(f(\nu)\) is real, non-negative and even. In particular, \(\gamma(0)=\int_{-1/2}^{1/2}f(\nu)d\nu\), so total spectral power equals process variance.
...
For a causal/invertible ARMA model \(\Phi(B)X_t=\Theta(B)E_t\), its spectral shape is
\[
\boxed{f(\nu)\propto\sigma_E^2
\frac{|\Theta(e^{-2\pi i\nu})|^2}{|\Phi(e^{-2\pi i\nu})|^2},}
\]
with the proportionality constant depending only on the spectral normalization convention.
...
\boxed{
\begin{array}{c}
\text{fit AR($p$)}\rightarrow\text{derive model-based spectrum}\\
\rightarrow\text{compare with smoothed periodogram}.
\end{array}
}
\]
...
\item Aliasing prevents identification above \(\nu=1/2\); physically \(f_N=1/(2\Delta t)\).
...
\item Spectrum and autocovariance are a Fourier pair; total power \(\int f=\gamma(0)\).
\item White Noise has flat spectrum.
\item AR(1) spectrum shape depends on sign/magnitude of \(\alpha\).
\item ARMA spectral shape is \(f(\nu)\propto\sigma_E^2|\Theta(e^{-2\pi i\nu})|^2/|\Phi(e^{-2\pi i\nu})|^2\).
...
\key{Goal.} Time-series classification (TSC) assigns one categorical label to an \emph{entire segment}, not to each time point. A supervised dataset is \(\{(X_i,Y_i)\}_{i=1}^N\), where a possibly multichannel window \(X_i\in\mathbb R^{d\times n}\) has class \(Y_i\in\{1,\ldots,C\}\). The classifier may output class probabilities \(\widehat p_c(X_i)\), with \(\widehat Y_i=\arg\max_c\widehat p_c(X_i)\).

\key{Windowing.} First choose one or more relevant window lengths; a long stream is split into fixed-length, possibly overlapping segments. Each window receives one label. Larger windows provide more temporal context but fewer training examples and may mix multiple events; shorter windows improve localization but may miss long-range structure. With overlapping windows, random window-level splitting causes leakage because neighboring segments share observations: split by independent unit (e.g. subject, machine, game) whenever possible.

\subsection{Nearest Neighbors and DTW}

\key{Euclidean \(k\)NN:} flatten each \(d\times n\) window, find the \(k\) training windows with smallest Euclidean distance and predict their majority class. It is simple, but treats equally indexed values as aligned, focuses strongly on level/amplitude and poorly handles time shifts, different speeds or lengths.

\key{Dynamic Time Warping (DTW)} aligns two sequences nonlinearly before measuring dissimilarity. With local cost \(d(x_i,z_j)\), cumulative cost satisfies
\[
\boxed{D(i,j)=d(x_i,z_j)+\min\{D(i-1,j),D(i,j-1),D(i-1,j-1)\}.}
\]
The optimal monotone warping path allows one point to match several points while preserving time order; \(D(n,m)\) is then used as the distance in \(k\)NN. DTW is useful for temporal misalignment, speed/timing differences and unequal lengths, but is computationally heavier; normalization may still be needed for level/amplitude differences.

\subsection{Feature-Based Machine Learning}

For each window, convert the sequence into a fixed feature vector and fit an ordinary classifier such as Random Forest or SVM. Useful features, computed on the original and possibly differenced, filtered or detrended signal, include:
\begin{itemize}
\item mean, variance, skewness, kurtosis and normality measures;
\item ACF values or sums of squared ACF coefficients over selected lags;
\item AR order selected by AIC, AR coefficients, innovation variance and extreme residuals;
\item trend slope, trend \(R^2\), residual trend error and measures of trend importance;
\item frequency-domain features such as dominant spectral frequencies/powers.
\end{itemize}
Several window lengths can be used in parallel. Feature engineering injects domain knowledge and works with smaller datasets, but depends on choosing informative, robust features; non-stationarity may require differencing/detrending or features designed to capture it.

\subsection{Deep Learning for TSC}

Deep networks can receive raw multichannel windows and learn representations automatically, so manual feature extraction is not required:
\begin{itemize}
\item \key{RNN/LSTM}: recurrent state models temporal dynamics; LSTM gates help retain long-term dependencies.
\item \key{1D-CNN}: convolution filters slide over time; stacked/dilated layers enlarge the receptive field and learn local-to-long-range patterns for fixed-length sequences.
\item \key{2D-CNN}: first transform signals into time--frequency images (e.g. spectrograms), then apply image-style convolutions.
\end{itemize}
The last layer produces a probability vector over classes. No architecture is universally best: in the football example, convolutional models generalized better than LSTMs, but this is an empirical result rather than a general theorem.

\subsection{Evaluation and Practical Issues}

Use a confusion matrix and class-specific precision/recall, not accuracy alone. Strong class imbalance (e.g. many more ``no action'' windows) can create many false positives/negatives; class-weighted loss or balanced sampling can help. Label noise and ambiguous/mislabeled events impose an upper limit on attainable performance. Always assess generalization on genuinely independent sequences and check adaptivity when the data-generating mechanism changes.

\[
\boxed{\begin{array}{c}
\text{segment}\rightarrow\text{Euclidean/DTW}\rightarrow k\text{NN}\\
\text{segment}\rightarrow\text{features}\rightarrow\text{RF/SVM}\\
\text{segment}\rightarrow\text{raw/spectrogram}\rightarrow\text{CNN/RNN}\\
\hfill\rightarrow\text{class probabilities}
\end{array}}
\]
